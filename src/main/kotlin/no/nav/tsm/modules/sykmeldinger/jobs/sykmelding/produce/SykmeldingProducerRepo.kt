...
class SykmeldingProducerRepo {
    ...
    fun getNext(): SykmeldingStatusJob? = transaction {
        exec(
            """
    ...
    fun resetHangingJobs(timestamp: OffsetDateTime): Int = transaction {
        SykmeldingStatusTable.update({
            (SykmeldingStatusTable.status inList
                listOf(SykmeldingStatusStatus.SENDING.name, SykmeldingStatusStatus.FAILED.name)) and
    ...
    fun updateStatus(sykmeldingId: UUID, failed: SykmeldingStatusStatus) = transaction {
