...
class SykmeldingExposedRepo() {
    fun test() {
        transaction {
            val sykmeldinger: List<Pair<Uuid, SykmeldingJsonb>> = SykmeldingExposed.selectAll().where {
                SykmeldingExposed.createdAt lessEq Clock.System.now()
    ...
    fun createBoio() {
        transaction {
            ...
            SykmeldingExposed.insert {
                ...
                it[data] = SykmeldingJsonb(
                    sykmeldingId = uuid.toString()
                )
            }
        }
    }
